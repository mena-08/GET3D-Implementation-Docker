...
def inference(
    ...
    grid_z = [torch.randn(1, G.z_dim, device=device, requires_grad=True) for _ in range(n_shape)]
    grid_tex_z = [torch.randn(1, G.z_dim, device=device, requires_grad=True) for _ in range(n_shape)]
