...
# MODIFIED BY OMAR MENA, MIT LICENSE AGREEMENT, AS NVIDIA 
...
def gen_swap(ws_geo_list, ws_tex_list, camera, generator, save_path, text_description, gen_mesh=False, ):
    '''
    ...
    with torch.no_grad():
        ...
        for i in range(len(select_geo_codes) - 1):
            ...
            gen_swap(
                ...
                save_path=save_path, text_description='a red chair',gen_mesh=gen_mesh
            )

def save_visualization(
        G_ema, grid_z, grid_c, run_dir, cur_nimg, grid_size, cur_tick,
        image_snapshot_ticks=50, save_gif_name=None, save_all=True, grid_tex_z=None):
    '''
    Save visualization during training, modified to use CLIP for text-guided image generation.
    
    :param G_ema: GET3D generator, assumed to be set up for inference.
    :param grid_z: Latent codes for geometry.
    :param grid_c: Conditioning codes (if any, otherwise pass None).
    :param run_dir: Directory to save outputs.
    :param cur_nimg: Current image number, used for logging and saving.
    :param grid_size: Dimensions for grid of generated images.
    :param cur_tick: Current training tick, used for periodic logging.
    :param image_snapshot_ticks: Interval for saving images.
    :param save_gif_name: Optional name for saving GIFs.
    :param save_all: Flag to save every image or selective ones.
    :param grid_tex_z: Latent codes for texture, if different from geometry.
    '''
    #device = G_ema.device
    ...
    for i_camera, camera in enumerate(camera_list):
        ...
        for z, geo_z, c in zip(grid_tex_z, grid_z, grid_c):
            ...
            geo_z.requires_grad_(True)
            #print("z :", type(z))
            #print("geo_z :", type(geo_z))
            optimizer = torch.optim.Adam([z, geo_z], lr=0.01)

            # Generate image and compute CLIP loss to guide the latent space
            optimizer.zero_grad()
            img, mask, _, _, _, mesh_v, mesh_f, _, _, _ = G_ema.generate_3d(
                z=z, geo_z=geo_z, c=c, noise_mode='const',
                generate_no_light=True, truncation_psi=0.2, camera=camera)

            img_pil = to_pil(img[0, :3, :, :])
            img_clip = preprocess(img_pil).unsqueeze(0).to(device)
            image_features = clip_model.encode_image(img_clip).to(device)
        ...
        if save_all:
            img = save_image_grid(images, None, drange=[-1, 1], grid_size=grid_size)
        else:
            save_file_name = f'fakes_{cur_nimg // 1000:06d}_{i_camera:02d}.png' if save_gif_name is None else 'fakes_%s' % save_gif_name.split('.')[0]
            img = save_image_grid(images, os.path.join(run_dir, save_file_name), drange=[-1, 1], grid_size=grid_size)
        camera_img_list.append(img)

    # Save a selection of 3D shapes for checking performance
    if cur_tick % min((image_snapshot_ticks * 20), 100) == 0:
        save_3d_shape(mesh_v_list[:10], mesh_f_list[:10], run_dir, cur_nimg // 10)
